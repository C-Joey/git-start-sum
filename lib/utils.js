 ...
 */
export function timeAgo(dateStr, translator = (key, substitutions) => chrome.i18n.getMessage(key, substitutions)) {
  const date = new Date(dateStr);
  const now = new Date();
  const seconds = Math.floor((now - date) / 1000);

  const intervals = [
    { type: 'Years', seconds: 31536000 },
    { type: 'Months', seconds: 2592000 },
    { type: 'Days', seconds: 86400 },
    { type: 'Hours', seconds: 3600 },
    { type: 'Minutes', seconds: 60 },
  ];

  for (const interval of intervals) {
    const count = Math.floor(seconds / interval.seconds);
    if (count >= 1) return translator(`timeAgo${interval.type}`, [count.toString()]);
  }
  return translator('timeAgoJustNow');
}
