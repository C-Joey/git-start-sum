...
async function apiRequest(path, options = {}) {
    ...
    if (!settings.githubToken) {
        throw new Error(chrome.i18n.getMessage('githubApiNoToken'));
    }
    ...
    if (contentType.toLowerCase().includes('text/html')) {
        throw new Error(chrome.i18n.getMessage('githubApiHtmlChallenge', [String(response.status)]));
    }
    ...
    if (response.status === 403) {
        ...
        if (remaining === '0') {
            ...
            throw new Error(chrome.i18n.getMessage('githubApiRateLimit', [resetTime.toLocaleTimeString()]));
        }
    }

    if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body.message || chrome.i18n.getMessage('githubApiError', [String(response.status)]));
    }

    // Handle 204 No Content (e.g. star/unstar)
    if (response.status === 204) return null;
    if ((options.method || 'GET').toUpperCase() === 'HEAD') return null;
...
export async function preflightGitHubApi() {
    return apiRequest('/user', { method: 'HEAD' });
}
...
export async function createSyncRepo(repoName) {
    return apiRequest('/user/repos', {
        ...
        body: JSON.stringify({
            name: repoName,
            description: chrome.i18n.getMessage('githubApiSyncRepoDesc'),
            private: true,
            auto_init: true,
        }),
    });
