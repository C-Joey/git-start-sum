...
const DEFAULT_SETTINGS = {
    ...
    aiApiMode: 'responses', // 'chat' | 'responses'
    aiApiKey: '',
    aiCustomEndpoint: '',
    aiCustomModel: '',
    autoSummary: true,
    ...
    appLanguage: 'system', // 'system' | 'zh_CN' | 'en'
};
...
export async function getCompleteStarsData() {
    const [starsData, cachedStars] = await Promise.all([
        getStarsData(),
        getCachedStars(),
    ]);
    const completeData = { ...starsData };

    for (const star of cachedStars) {
        const fullName = star?.fullName;
        if (!fullName) continue;

        completeData[fullName] = {
            ...(completeData[fullName] || {}),
            repoInfo: star,
            starredAt: completeData[fullName]?.starredAt || star.starredAt,
        };
    }
...
export async function exportAllData() {
    const [starsData, cachedStars, tags, history, settings] = await Promise.all([
        getCompleteStarsData(),
        getCachedStars(),
        getTags(),
        getHistory(),
        getSettings(),
    ]);
    return { starsData, cachedStars, tags, history, settings, exportedAt: new Date().toISOString() };
}
