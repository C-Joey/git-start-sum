...
export async function syncToGitHub() {
    ...
    await putRepoFile(
        ...
        'data.json',
        jsonData,
        `📦 Sync stars data — ${new Date().toLocaleString('zh-CN')}`,
        existingJson?.sha
    );
    ...
    await putRepoFile(
        ...
        'README.md',
        markdown,
        `📝 Update stars README — ${new Date().toLocaleString('zh-CN')}`,
        existingReadme?.sha
    );
    ...
    if (history.length > 0) {
        ...
        await putRepoFile(
            ...
            'HISTORY.md',
            historyMd,
            `📅 Update browsing history — ${new Date().toLocaleString('zh-CN')}`,
            existingHistory?.sha
        );
    }
...
function generateMarkdown(starsData, tags) {
    ...
    let md = `# ⭐ My GitHub Stars\n\n`;
    md += `> 由 [GitHub Stars Manager](https://github.com) 自动生成 | 共 ${entries.length} 个项目\n\n`;
    md += `---\n\n`;
...
export function generateShareMarkdown(selectedStars, starsData, title = '我的推荐项目') {
    let md = `# ⭐ ${title}\n\n`;
    md += `> 精选自我的 GitHub Stars | 共 ${selectedStars.length} 个项目\n\n---\n\n`;
